...
describe('シナリオ: 小型ステージを自動走行・自動ジャンプでゴールまで到達できる', () => {
  ...
  it('自動ジャンプでは越えられない穴でも、地形を生成すれば橋渡ししてゴールに到達できる', () => {
    ...
    const bridgeTerrain: TerrainDefinition = { id: 'bridge', name: '橋', cost: 5, unlocked: true, grid: ['NNNNNNNN'] };
  ...
  it('トゲに触れてHPが0になると、チェックポイントへ復帰する(HP全快・生成地形とマナは維持・敵は初期配置リセット)', () => {
    ...
    const decorTerrain: TerrainDefinition = { id: 'deco', name: '飾り', cost: 1, unlocked: true, grid: ['N'] };
